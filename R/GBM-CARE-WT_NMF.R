...
load_nmf_results <- function(path, samples = NULL) {
    ...
    if (!is.null(samples)) {
        snames <- sapply(strsplit(nmf_files, split = "_"), function(x) {
            x[[1]][1]
        })
    ...
    nmf_res_list <- lapply(nmf_files, function(x) {
        readRDS(file = paste0(path, x))
    })
    names(nmf_res_list) <- nmf_files

    snames <- sapply(strsplit(names(nmf_res_list), split = "_"), function(x) {
        x[[1]][1]
    })

    Genes_nmf_w_basis <- lapply(1:length(nmf_res_list), function(i) {
        get_nmf_programs(nmf_res_list[[i]]$fit, names(nmf_res_list)[i])
    })
...
derive_NMF_metaprograms <- function(
    Genes_nmf_w_basis,
    n_genes = 50,
    save_path = "",
    plot = T,
    ...
    nmf_programs_sig <- lapply(Genes_nmf_w_basis, function(x) {
        apply(x, 2, function(y) names(sort(y, decreasing = T))[1:n_genes])
    })
    ...
    nmf_programs_sig <- lapply(nmf_programs_sig, function(x) {
    ...
    nmf_intersect <- apply(nmf_programs_sig, 2, function(x) {
        apply(nmf_programs_sig, 2, function(y) length(intersect(x, y)))
    })

    # hierarchical clustering of the similarity matrix
    nmf_intersect_hc_ccle <- hclust(
        as.dist(n_genes - nmf_intersect),
        method = "average"
    )
    nmf_intersect_hc_ccle <- reorder(
        as.dendrogram(nmf_intersect_hc_ccle),
        colMeans(nmf_intersect)
    )
    nmf_intersect <- nmf_intersect[
        order.dendrogram(nmf_intersect_hc_ccle),
        order.dendrogram(nmf_intersect_hc_ccle)
    ]
    ...
    Sorted_intersection <- sort(
        apply(nmf_intersect, 2, function(x) {
    ...
    while (Sorted_intersection[1] > Min_group_size) {
        ...
        nmf_programs_sig <- nmf_programs_sig[,
            -match(names(Sorted_intersection[1]), colnames(nmf_programs_sig))
        ] # remove selected NMF
        Intersection_with_Genes_MP <- sort(
            apply(nmf_programs_sig, 2, function(x) {
        ...
        while (Intersection_with_Genes_MP[1] >= Min_intersect_cluster) {
            ...
            Curr_cluster <- c(
                Curr_cluster,
                names(Intersection_with_Genes_MP)[1]
            )

            Genes_MP_temp <- sort(
                table(c(
                    NMF_history,
                    nmf_programs_sig[, names(Intersection_with_Genes_MP)[1]]
                )),
                decreasing = TRUE
            ) ## Genes_MP is newly defined each time according to all NMFs in the current cluster
            Genes_at_border <- Genes_MP_temp[which(
                Genes_MP_temp == Genes_MP_temp[n_genes]
            ...
            if (length(Genes_at_border) > 1) {
                ...
                for (i in Curr_cluster) {
                    ...
                    Q <- Genes_nmf_w_basis[[curr_study]][
                        match(
                            ...
                            !is.na(match(
                                names(Genes_at_border),
                                toupper(rownames(Genes_nmf_w_basis[[
                                    curr_study
                                ]]))
                            ))
                        ],
                ...
                Genes_curr_NMF_score_sort <- sort(
                    Genes_curr_NMF_score,
                    decreasing = TRUE
                )
                Genes_curr_NMF_score_sort <- Genes_curr_NMF_score_sort[unique(names(
                    Genes_curr_NMF_score_sort
                ))] ## take only the maximal score of each gene - which is the first entry after sorting

                Genes_MP_temp <- c(
                    names(Genes_MP_temp[which(
                        Genes_MP_temp > Genes_MP_temp[n_genes]
                    )]),
    ...
    for (j in 1:length(Cluster_list)) {
        inds_sorted <- c(
            inds_sorted,
            match(Cluster_list[[j]], colnames(nmf_intersect_KEEP))
        )
    }
    inds_new <- c(
        inds_sorted,
        which(is.na(match(1:dim(nmf_intersect_KEEP)[2], inds_sorted)))
    ) ### combine inds in clusters with inds without clusters

    # plot re-ordered similarity matrix heatmap
    nmf_intersect_meltI_NEW <- reshape2::melt(nmf_intersect_KEEP[
        inds_new,
        inds_new
    ])
    ...
    if (plot == T) {
        ...
        p <-
            ggplot(
                data = nmf_intersect_meltI_NEW,
                aes(
                    x = Var1,
                    y = Var2,
                    fill = 100 * value / (100 - value),
                    color = 100 * value / (100 - value)
                )
            ) +
            geom_tile() +
            scale_color_gradient2(
                limits = c(2, 25),
                low = custom_magma[1:111],
                mid = custom_magma[112:222],
                high = custom_magma[223:333],
                midpoint = 13.5,
                oob = squish,
                name = "Similarity\n(Jaccard index)"
            ) +
    ...
    write.csv(
        MP,
        file = paste0(save_path, "Meta_Programs_generated_automatically.csv")
    )
...
metaprograms_enrichment <- function(
    mp_list,
    pathways = NULL,
    min_gs_size = 40,
    max_gs_size = 500
    ...
    pathways <- pathways[
        lengths(pathways) >= min_gs_size & lengths(pathways) <= max_gs_size
    ]
    length(pathways)
    ...
    path_en$int_genes <- sapply(1:nrow(path_en), function(i) {
        intersect(pathways[[path_en$Pathway[i]]], mp_list[[path_en$MP[i]]])
    })
...
get_top_genes <- function(fit, rank, sname, n = 50) {
    ...
    res <- lapply(1:ncol(w), function(i) {
        sort(w[, i], decreasing = T) %>% names()
    })
...
robust_nmf_programs <- function(
    ...
    intra_intersect <- lapply(nmf_programs, function(z) {
        apply(z, 2, function(x) {
            apply(z, 2, function(y) length(intersect(x, y)))
        })
    })
    intra_intersect_max <- lapply(intra_intersect, function(x) {
        apply(x, 2, function(y) sort(y, decreasing = T)[2])
    })
    ...
    for (i in names(nmf_sel)) {
        a <- inter_intersect[
            grep(i, colnames(inter_intersect), invert = T),
        ...
        if (length(b) > 1) {
            ...
            for (y in 2:length(b)) {
                if (max(inter_intersect[c, names(b[y])]) <= intra_max) {
                    c <- c(c, names(b[y]))
                } # selects programs iteratively from top-down. Only selects programs that have a intersection smaller than 10 with a previously selected programs
            }
