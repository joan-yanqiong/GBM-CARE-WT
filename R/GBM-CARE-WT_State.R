...
norm_fit <- lapply(colnames(permuted_data_merged), function(mp) tibble(MP = mp,
                                                                       Sig = rnorm(n = 100000,
                                                                                   mean = mean_vec[mp],
                                                                                   sd = sd_vec[mp])))
norm_fit <- do.call(rbind, norm_fit)

dm <- rbind(melt(data = permuted_data_merged,
                 measure.vars = vars) %>% mutate(DataType = "Permuted"),
            melt(data = mdata %>% select(starts_with("MP")),
...
table(cc_data$Timepoint, cc_data$isCC) / rowSums(table(cc_data$Timepoint, cc_data$isCC))

cc_data <- cc_data %>%
  mutate(CCstate = case_when(p.adj < .05 ~ "Cycling",
                             p.val < .05 ~ "Borderline",
                             TRUE ~ "Non-cycling")) %>%
  mutate(CCstate = factor(CCstate, c("Non-cycling", "Borderline", "Cycling")))

table(cc_data$Timepoint, cc_data$CCstate)
table(cc_data$Timepoint, cc_data$CCstate) / rowSums(table(cc_data$Timepoint, cc_data$CCstate))
...
ggplot(cc_data, aes(x = Timepoint, y = MP_3_CC, color = CCstate)) + 
  ...
  scale_color_manual(name = "", values = c("Non-cycling" = "dodgerblue", "Borderline" = "black", "Cycling" = "red")) +
  coord_cartesian(xlim = c(1.2, NA)) +
  xlab("") +
  ylab("Cell cycle score") +
  scale_y_continuous(breaks = seq(-.5, 1.5, .25), limits = c(-.5, 1.5), oob = squish) +
  theme_gbm_pvsr() +
  theme(panel.grid.major = element_line())

cc_data$log10Padj <- -log10(cc_data$p.adj)

ggscatter(data = cc_data, x = "MP_3_CC", y = "log10Padj", color = "CCstate", facet.by = "Timepoint") +
  theme_gbm_pvsr() +
  theme(panel.grid.major = element_line())
...
table(mdata$Timepoint, mdata$State) / rowSums(table(mdata$Timepoint, mdata$State))
...
mdata %>%
  ...
  xlab("") +
  ylab("Complexity") +
  geom_hline(yintercept = median(mdata$nFeature_RNA), linetype = "dashed", size = 1, color = "black") +
...
mp_class <- c("MP_1_RP" = "LQ", "MP_2_OPC" = "OPC", "MP_4_AC" = "AC", "MP_5_Hypoxia" = "Hypoxia",
              "MP_6_MES" = "MES", "MP_7_NPC" = "NPC", "MP_8_GPC" = "GPC", "MP_9_ExN" = "Neuron", 
              "MP_10_Stress1" = "Stress", "MP_11_MIC" = "Doublet", "MP_12_LQ" = "LQ", "MP_13_Cilia" = "Cilia",
              "MP_14_NRGN" = "Neuron", "MP_15_Stress2" = "Stress")
table(mp_class)
...
state_data_hybrid$Sample <- sapply(1:nrow(state_data_hybrid), function(i) get_sname(state_data_hybrid$CellID[i], "-"))
...
max_prog_class <- sdata_arranged %>%
  group_by(CellID) %>%
  summarise(H1 = first(Program), H2 = nth(Program, 2),
            S1 = first(Score), S2 = nth(Score, 2), Sdiff = abs(S1 - S2),
            Class1 = mp_class[H1], Class2 = mp_class[H2])
...
max_prog_class <- max_prog_class %>%
  ungroup() %>%
  mutate(FinalClass = case_when(is.na(Class2)       ~ Class1,
...
state_data_hybrid <- state_data %>% filter(p.sig == T, Class %ni% c("LQ", "Doublet")) %>% group_by(CellID) %>% summarise(N = n())
...
sdata_arranged <- state_data %>%
  ungroup() %>%
  filter(p.sig == T, Class %ni% c("LQ", "Doublet"), CellID %in% state_data_hybrid$CellID) %>%
  group_by(CellID) %>%
  arrange(CellID, desc(Score))
dim(sdata_arranged)
...
max_prog_class <- sdata_arranged %>%
  group_by(CellID) %>%
  summarise(H1 = first(Program), H2 = nth(Program, 2),
            S1 = first(Score), S2 = nth(Score, 2), Sdiff = S1 - S2,
            Class1 = mp_class[H1], Class2 = mp_class[H2])

max_prog_class <- max_prog_class %>% filter(CellID %in% state_data_hybrid$CellID)
dim(max_prog_class)
...
dm <- rbind(max_prog_class %>%
              select(Sdiff) %>%
              mutate(DataType = "Observed"),
            tibble(Sdiff = permuted_diffs, DataType = "Expected"))

ggplot(data = dm, aes(x = Sdiff, y = after_stat(ncount), color = DataType, linetype = DataType)) +
  geom_freqpoly(bins = 100, size = 1, show.legend = c(color = T, linetype = F)) +
  scale_color_manual(name = "", values = c("Observed" = "red", "Expected" = "black")) +
  scale_linetype_manual(values = c("Observed" = "solid", "Expected" = "dashed")) +
  scale_fill_discrete(name = "Data distribution") +
  geom_vline(xintercept = quantile(permuted_diffs, .95), linetype = "dashed", size = 1, color = "black") +
...
ggplot(data = dm, aes(x = SdiffWeight, y = after_stat(ncount), color = DataType, linetype = DataType)) +
  geom_freqpoly(bins = 100, size = 1, show.legend = c(color = T, linetype = F)) +
  scale_color_manual(name = "", values = c("Observed" = "red", "Expected" = "black")) +
  scale_linetype_manual(values = c("Observed" = "solid", "Expected" = "dashed")) +
  scale_fill_discrete(name = "Data distribution") +
  geom_vline(xintercept = quantile(permuted_diffs_weight, .95), linetype = "dashed", size = 1, color = "black") +
  xlab("Distribution of score differences") +
  ylab("Count (scaled to 1)") + 
  theme_gbm_pvsr(panel.border = element_blank()) +
  theme(panel.grid.major = element_line())
...
max_prog_class <- max_prog_class %>%
  ungroup() %>%
  mutate(isHybrid = !is.na(Class2) & Class1 != Class2 & Sdiff < quantile(permuted_diffs, .95) & SdiffWeight < .5)
table(max_prog_class$isHybrid)
table(max_prog_class$isHybrid) / nrow(max_prog_class)

max_prog_class %>% filter(isHybrid == T) %>% dplyr::select(Class1, Class2) %>% table()
(max_prog_class %>% filter(isHybrid == T) %>% dplyr::select(Class1, Class2) %>% table()) / nrow(max_prog_class %>% filter(isHybrid == T)) * 100
...
table(mdata$Timepoint, mdata$State) / rowSums(table(mdata$Timepoint, mdata$State)) * 100
table(mdata$isCC, mdata$Timepoint)
table(mdata$Timepoint, mdata$isCC) / rowSums(table(mdata$Timepoint, mdata$isCC)) * 100
