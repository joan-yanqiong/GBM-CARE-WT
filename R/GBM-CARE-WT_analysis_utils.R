...
celltype_color_vec <- c(
    "Malignant" = "#FB8072",
    "Oligodendrocyte" = "#B3DE69",
    "Macrophage" = "#80B1D3", # 4, 7, 5
    "Astrocyte" = "purple",
    "Excitatory neuron" = "#BC80BD",
    "Inhibitory neuron" = "#FFED6F",
    "Other neuron" = "#CCEBC5", # 6, 10, 12
    "Endothel" = "#FCCDE5",
    "Pericyte" = "#FFFFB3",
    "Tcell" = "#8DD3C7",
    "Bcell" = "#BEBADA",
    "OPC" = "#FDB462", # 8, 2, 1, 3, 11
    "Other normal" = "#D9D9D9",
    "Unresolved" = "grey"
) # 10

celltype_color_vec_reduced <- c(
    "Malignant" = "#FB8072",
    "Oligodendrocyte" = "#B3DE69",
    "TAM" = "#80B1D3",
    "Astrocyte" = "#BEBADA",
...
select_genes <- function(
    ...
    # he_genes <- plapply(X = unique(md$Sample),
    #                     FUN = function(sname) {
    #                       m <- upm_data_list[[sname]]
    #                       m <- m[, md$CellID[md$Sample == sname]]
    #                       heg <- log2(rowMeans(m) + 1)
    #                       rownames(m)[heg > exp_th]
    #                     },
    #                     pack = F)
    he_genes <- lapply(X = unique(md$Sample), FUN = function(sname) {
    ...
    # ext_genes <- plapply(X = unique(md$Sample),
    #                      FUN = function(sname) {
    #                        m <- upm_data_list[[sname]]
    #                        m <- m[, md$CellID[md$Sample == sname]]
    #                        m <- m > umi_th
    ...
    #                      },
    #                      pack = F)
    ext_genes <- lapply(X = unique(md$Sample), FUN = function(sname) {
        m <- umi_data_list[[sname]]
        m <- m[, md$CellID[md$Sample == sname]]
        m <- m > umi_th
        rowSums(m)
    })
    he_genes2 <- unname(unlist(
        lapply(ext_genes, function(eg) names(eg)[eg >= cells_th]),
        recursive = F
    ))
...
doublet_detection_null_dist <- function(
    m,
    cluster_means,
    frac_sampled = .1,
    n_iter = 100,
    ...
    null_dist <- lapply(1:n_iter, function(i) {
        ...
        colnames(m_perm) <- sampled_cells
        rownames(m_perm) <- rownames(m)
...
plot_patients_butterfly <- function(data, patients) {
    ggplot(
        data = data %>% filter(Patient %in% patients),
        mapping = aes(x = Dx, y = Dy)
    ) +
        facet_grid(cols = vars(Patient_factor), rows = vars(Timepoint)) +
        # facet_grid(cols = vars(ID)) +
        geom_point(
            mapping = aes(color = State, fill = State, alpha = State),
            shape = 21,
            color = "black",
            size = 5,
            show.legend = c("color" = TRUE, "fill" = TRUE, "alpha" = FALSE)
...
add_glioma_sigs <- function(pathways) {
    ...
    pathways$GLIOMA_G1S <- unique(c(
        scandal::SCANDAL_IDH_O_G1S_MARKERS,
        scandal::SCANDAL_IDH_A_G1S_MARKERS,
        scalop::Signatures_GBM$G1S
    ))
    pathways$GLIOMA_G2M <- unique(c(
        scandal::SCANDAL_IDH_O_G2M_MARKERS,
        scandal::SCANDAL_IDH_A_G2M_MARKERS,
        scalop::Signatures_GBM$G2M
    ))
...
load_c8_pathways <- function(path = "data/msigdb/") {
...
default_pathways <- function(path = "/data/msigdb/") {
    ...
    pathways <- c(
        pathways,
        fgsea::gmtPathways("data/msigdb/h.all.v7.4.symbols.gmt")
    )

    pathways <- c(
        pathways,
        fgsea::gmtPathways("data/msigdb/c5.go.bp.v7.4.symbols.gmt")
    )

    pathways <- c(
        pathways,
        fgsea::gmtPathways("data/msigdb/c5.go.cc.v7.4.symbols.gmt")
    )

    pathways <- c(
        pathways,
        fgsea::gmtPathways("data/msigdb/c5.go.mf.v7.4.symbols.gmt")
    )

    pathways <- c(
        pathways,
        fgsea::gmtPathways("data/msigdb/c2.cp.reactome.v7.4.symbols.gmt")
    )
    ...
    pathways <- c(
        pathways,
        list(Synaptic = readRDS("integrated_analysis3/data/syn_sig.RDS"))
    )
...
load_brain_signatures <- function() {
    ...
    sigs_list <- list(
        Macrophage = derive_markers(load_c8_pathways(), "MACROPHAGE|MICROGLIA"),
        Tcell = unique(c(
            derive_markers(load_c8_pathways(), "_T_CELL|_NK"),
            "CD2",
            "SKAP1",
            "ITK",
            "ICOS",
        ...
        Bcell = unique(c(
            derive_markers(load_c8_pathways(), "_B_CELL|_PLASMA", n_rep = 3),
            "IGKC",
            "IGHM",
            "FCRL1",
            "FCRL5",
...
dT_state <- function(md) {
    ...
    t1_m <- lapply(states, function(s) {
        res_m %>% filter(Timepoint == "T1", State == s) %>% pull(Freq)
    })
    t1_m <- t(do.call(rbind, t1_m))
    colnames(t1_m) <- paste0("T1_ABS_State_", states)
    rownames(t1_m) <- pts

    t2_m <- lapply(states, function(s) {
        t2 <- res_m %>% filter(Timepoint == "T2", State == s) %>% pull(Freq)
    })
    ...
    res_m <- list(dT = t2_vs_t1, T1_ABS = t1_m, T2_ABS = t2_m)
...
dTME_celltype <- function(md, celltype_ext = F) {
    ...
    t1_m <- lapply(states, function(s) {
        res_m %>% filter(Timepoint == "T1", CellType == s) %>% pull(Freq)
    })
    t1_m <- t(do.call(rbind, t1_m))
    ...
    t2_m <- lapply(states, function(s) {
        t2 <- res_m %>% filter(Timepoint == "T2", CellType == s) %>% pull(Freq)
    })
    t2_m <- t(do.call(rbind, t2_m))
